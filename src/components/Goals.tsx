    ...
    <div className="space-y-6">
      <Card>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
          <Input value={name} onChange={setName} placeholder="목표 이름 (예: 내집마련)" />
          <MoneyInput value={target} onChange={setTarget} placeholder="목표 금액(원)" />
          <Input type="date" value={date} onChange={setDate} />
          <div>
            <label className="text-xs text-neutral-400">현재 보유(초기금)</label>
            <MoneyInput value={initial} onChange={setInitial} />
          </div>
          <div>
            <label className="text-xs text-neutral-400">기대수익률 %</label>
      ...
      <div className="space-y-3">
        {goals.length === 0 && (
          <p className="py-8 text-center text-sm text-neutral-400">등록된 목표가 없어요</p>
        )}
        {goals.map((g) => (
  ...
  return (
    <Card>
      <div className="flex items-start justify-between">
        ...
        <button onClick={onRemove} className="text-neutral-300 hover:text-rose-500">✕</button>
      </div>
      ...
      <div className="mt-3 rounded-lg bg-neutral-50 p-3 text-sm">
        {months <= 0 ? (
          <span className="text-rose-600">목표일이 지났어요. 날짜를 조정하세요.</span>
        ) : reachableByInitial ? (
          ...
          <span>
            매월 <span className="font-bold text-blue-600">{formatKRW(need)}원</span>씩 적립하면
