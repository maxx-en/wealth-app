      ...
      <div className="w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        <Card>
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-lg font-bold">설정 · 가족 공유</h2>
            <button onClick={onClose} className="text-neutral-400 hover:text-neutral-700">✕</button>
          </div>

          {/* 내 계정 */}
          <div className="mb-5 flex items-center gap-3 rounded-lg bg-neutral-50 p-3">
            ...
            <Button variant="ghost" onClick={() => signOut({ callbackUrl: "/login" })}>로그아웃</Button>
          </div>

          {/* 가구 구성원 */}
          <div className="mb-5">
            <h3 className="mb-2 text-sm font-semibold">우리 가계부 구성원 ({data?.members.length ?? 0}명)</h3>
          ...
          <div className="mb-5 border-t border-neutral-100 pt-4">
            ...
            <p className="mb-2 text-xs text-neutral-400">
              아래 코드를 가족에게 알려주세요. 가족이 로그인 후 “코드로 합류”에 입력하면 같은 가계부를 함께 봐요.
            </p>
            {inviteCode ? (
              <div className="flex items-center gap-2">
                <div className="flex-1 rounded-lg border border-neutral-200 bg-neutral-50 px-3 py-2 text-center font-mono text-lg font-bold tracking-widest">
                  {inviteCode}
                </div>
                <Button variant="ghost" onClick={() => { navigator.clipboard.writeText(inviteCode); setMsg("코드 복사됨"); }}>복사</Button>
              </div>
            ) : (
              <Button onClick={makeCode} className="w-full">초대 코드 만들기</Button>
            )}
          </div>
          ...
          <div className="border-t border-neutral-100 pt-4">
            ...
            <p className="mb-2 text-xs text-neutral-400">
              가족에게 받은 초대 코드를 입력하면 그 가계부로 합류해요. (지금 내 데이터는 더 이상 안 보이게 됩니다)
            </p>
            <div className="flex items-center gap-2">
              <Input value={joinCode} onChange={(v) => setJoinCode(v.toUpperCase())} placeholder="초대 코드 입력" />
